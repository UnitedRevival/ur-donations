...
const DonationPayments = () => {
  ...
  const fetchRecentDonations = async () => {
    try {
      ...
      const response = await axios.get(`/api/recentDonations?date=${date}&donationType=Jesus March 2026 - Washington Dc`);
  ...
  const { channel } = useChannel('payments', 'newPayment', (message) => {
    ...
    const isTargetEvent = !donationType || donationType === "Jesus March 2026 - Washington Dc";
