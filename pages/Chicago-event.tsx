...
export default function ChicagoEvent({
  amountRaised,
  goal,
  cardTitle,
}: HomePageProps) {
...
export const getServerSideProps: GetServerSideProps<HomePageProps> = async (
  ...
  const jesusMarchDonations = totals.find(
    (t) => t._id === current_Diffrent_campaigns.JESUS_MARCH_2026_CHICAGO.title,
  );

  return {
    props: {
      amountRaised: jesusMarchDonations?.total || 0,
      goal: current_Diffrent_campaigns.JESUS_MARCH_2026_CHICAGO.goal,
      cardTitle: `Help Fund Jesus March Chicago ${CAMPAIGN_YEAR}`,
    },
  };
};
