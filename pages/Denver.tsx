...
export default function Denver({ amountRaised, goal, cardTitle }: HomePageProps) {
  return (
...
export const getServerSideProps: GetServerSideProps<HomePageProps> = async (ctx) => {
  ...
  const jesusMarchDonations = totals.find(
    (t) => t._id === current_Diffrent_campaigns.JESUS_MARCH_2025_DENVER.title
  );

  return {
    props: {
      amountRaised: jesusMarchDonations?.total || 0,
      goal: current_Diffrent_campaigns.JESUS_MARCH_2025_DENVER.goal,
      cardTitle: `Thanks For Signing-Up For Jesus March Denver ${CAMPAIGN_YEAR}`, // Pass it through props
