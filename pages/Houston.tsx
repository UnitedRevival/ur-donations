...
export default function Houston({ amountRaised, goal, cardTitle }: HomePageProps) {
  return (
...
export const getStaticProps: GetStaticProps<HomePageProps> = async (ctx) => {
  ...
  const jesusMarchDonations = totals.find(
    (t) => t._id === currentCampaigns.JESUS_MARCH_2025_HOUSTON.title
  );

  return {
    props: {
      amountRaised: jesusMarchDonations?.total || 0,
      goal: currentCampaigns.JESUS_MARCH_2025_HOUSTON.goal,
      cardTitle: 'Thanks For Signing-Up For Jesus March Houston 2025', // Pass it through props
