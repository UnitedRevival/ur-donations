...
export default function PortlandEvent({
  amountRaised,
  goal,
  cardTitle,
}: HomePageProps) {
...
export const getServerSideProps: GetServerSideProps<HomePageProps> = async (
  ...
  const jesusMarchDonations = totals.find(
    (t) => t._id === current_Diffrent_campaigns.JESUS_MARCH_2026_PORTLAND.title,
  );

  return {
    props: {
      amountRaised: jesusMarchDonations?.total || 0,
      goal: current_Diffrent_campaigns.JESUS_MARCH_2026_PORTLAND.goal,
      cardTitle: `Help Fund Jesus March Portland ${CAMPAIGN_YEAR}`,
    },
  };
};
