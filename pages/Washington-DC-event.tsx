...
export default function WashingtonDCEvent({ amountRaised, goal, cardTitle }: HomePageProps) {
  return (
...
export const getStaticProps: GetStaticProps<HomePageProps> = async (ctx) => {
  ...
  const jesusMarchDonations = totals.find(
    (t) => t._id === current_Diffrent_campaigns.JESUS_MARCH_2025_WASHINGTON_DC_EVENT.title
  );

  return {
    props: {
      amountRaised: jesusMarchDonations?.total || 0,
      goal: current_Diffrent_campaigns.JESUS_MARCH_2025_WASHINGTON_DC_EVENT.goal,
